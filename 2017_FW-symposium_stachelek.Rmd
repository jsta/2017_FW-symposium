...
## test
\center
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=5.0, out.height="160pt"}
milstead <- MRB1
...
# glm
ggplot(data = na.omit(dt[dt$Rp > 0,]), aes(x = 1/flushing.rate, y = Rp)) + geom_point() + scale_x_log10() + viridis::scale_color_viridis(discrete = TRUE, direction = -1) + xlab("Water Residence Time") + stat_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE) + facet_grid(. ~ source, scales = "free_x")
